...
BOOL grow_hfs(Volume* volume, uint64_t newSize) {
	...
	success &= !! setBlockUsed(volume, volume->volumeHeader->totalBlocks - 1, 0);
	...
	for(i = ((volume->volumeHeader->totalBlocks / 8) + 1); i < newMapSize; i++) {
		BOOL r = WRITE(volume->allocationFile, i, 1, &zero);
		//ASSERT(r, "WRITE");
		success &= !!r;
	}
	
	/* grow backing store size */
	BOOL r = WRITE(volume->image, newSize - 1, 1, &zero);
	success &= !!r;
	//ASSERT(r, "WRITE");
	...
	/* reserve last block */	
	success &= !! setBlockUsed(volume, volume->volumeHeader->totalBlocks - 1, 1);
	
	success &= !! updateVolume(volume);
	return success;
}
	...
	while(curRecord < tarSize) {
		...
		if(record) {
			if(record->recordType == kHFSPlusFolderRecord || type == 5) {
				...
				printf("replacing %s\n", fileName);
				free(record);
				success &= !!removeFile(fileName, volume);
			}
		}
		...
		if(type == 0) {
			...
			success &= !!add_hfs(volume, inFile, fileName);
			free(buffer);
		} else if(type == 5) {
			if(!silence)
				printf("directory: %s (%04o)\n", fileName, mode);
			success &= !!newFolder(fileName, volume);
		} else if(type == 2) {
			if(!silence)
				printf("symlink: %s (%04o) -> %s\n", fileName, mode, target);
			success &= !!makeSymlink(fileName, target, volume);
		}

		success &= !!chmodFile(fileName, mode, volume);
		success &= !!chownFile(fileName, uid, gid, volume);
