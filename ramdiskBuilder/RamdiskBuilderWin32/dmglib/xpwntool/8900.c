...
void close8900(AbstractFile* file) {
	...
	if(info->dirty) {
		...
		if(info->header.format == 3) {
			memset(ivec, 0, 16);
			AES_cbc_encrypt((const unsigned char*)info->buffer, (unsigned char*)info->buffer, info->header.sizeOfData, &(info->encryptKey), ivec, AES_ENCRYPT);
		}
...
AbstractFile* createAbstractFileFrom8900(AbstractFile* file) {
	...
	if(info->header.format == 3) {
		memset(ivec, 0, 16);
		AES_cbc_encrypt((const unsigned char*)info->buffer, (unsigned char*)info->buffer, info->header.sizeOfData, &(info->decryptKey), ivec, AES_DECRYPT);
	}
