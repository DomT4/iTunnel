...
void closeImg3(AbstractFile* file) {
	...
	if(info->dirty) {
		if(info->encrypted) {
			...
			AES_cbc_encrypt((const unsigned char*)info->data->data, (unsigned char*)info->data->data, (info->data->header->dataSize / 16) * 16, &(info->encryptKey), ivec, AES_ENCRYPT);
		}
...
void setKeyImg3(AbstractFile2* file, const unsigned int* key, const unsigned int* iv) {
	...
	if(!info->encrypted) {
		uint8_t ivec[16];
		memcpy(ivec, info->iv, 16);
		AES_cbc_encrypt((const unsigned char*)info->data->data, (unsigned char*)info->data->data, (info->data->header->dataSize / 16) * 16, &(info->decryptKey), ivec, AES_DECRYPT);
	}
