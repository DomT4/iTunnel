...
signed int getUsbDeviceName(void* thisObj, int interfaceType, char *Dest, size_t Count)
{
	...
	Log(LOG_INFO, "getUsbDeviceName: %s", Dest);
	return 0;
}
...
signed int __cdecl sub_10032290(void* a1, BYTE* a2)
{
	...
	*a2 = -1;
	...
	else
	{
		...
		Log(LOG_INFO, "interface has %d endpoints, file pipe = %d\n", v4, v3);
		result = 0;
	}
	return result;
}
...
signed int __stdcall sub_1003B150(const char *Source, LPVOID lpOutBuffer, void* a3)
{
	...
	Log(LOG_INFO, "WinDFU::OpenDeviceByPath: %s", Source);
...
signed int sub_1003B5E0(void *thisObj, const char *Source)
{
	...
	Log(LOG_INFO, "WinDFU::OpenDFUDevice: path: %s", Source);
...
int __cdecl sub_10039BF0(int a1, int a2, int a3)
{
	...
	v4 = *(_DWORD *)(a1 + 16);
	Log(LOG_INFO, "operation %d progress %d", v4, a3);
...
int __cdecl sub_1003AEA0(int a1, int a2, int a3)
{
	...
	Log(LOG_INFO, "operation %d progress %d", a2, a3);
	result = *(_DWORD *)(a1 + 8);
	if ( result )
	...
	while ( 2 )
	{
		...
		if ( NumberOfBytesRead < *(_WORD *)(v2 + 24) )
		{
			if ( v1 )
			{
				...
				v39 = v1 - v18;
				Log(LOG_INFO, "WinDFU::UploadData: EOF, cbRead: %d", v19);
				v4 = NumberOfBytesRead;
			}
		}
		if ( v4 )
...
signed int __stdcall WinDFU__ResetDevice(HANDLE hDevice)
{
	...
	if (g_exploitMode) {
		...
		Overlapped.Offset = 0;
		Overlapped.OffsetHigh = 0;
		Log(LOG_INFO, "WinDFU::ResetDevice: resetting...");
...
signed int  sub_1003B760(void *thisObj, char a2[6], int* a3, int* a4, size_t* a5)
{
	...
	{
	case 4:
		Log(LOG_INFO, "WinDFU::ProcessUpdateState: status.bState == DFU_STATE_DNBUSY");
...
LABEL_5:
		...
		break;
	case 8:
		Log(LOG_INFO, "WinDFU::ProcessUpdateState: status.bState == DFU_STATE_MANIFEST_WAIT_RESET");
		WinDFU__ResetDevice(*((HANDLE *)v5 + 10));
		*a4 = 1;
		result = 0;
		break;
	case 2:
		Log(LOG_INFO, "WinDFU::ProcessUpdateState: status.bState == DFU_STATE_DFU_IDLE");
		result = 0;
		*a4 = 1;
		break;
	case 6:
		Log(LOG_INFO, "WinDFU::ProcessUpdateState: status.bState == DFU_STATE_MANIFEST_SYNC");
		goto LABEL_9;
	case 5:
		Log(LOG_INFO, "WinDFU::ProcessUpdateState: status.bState == DFU_STATE_DNLOAD_IDLE");
LABEL_9:
		result = 0;
		break;
	default:
...
int uploadFileDfu(AMRecoveryModeDevice device, const char* fileName)
{
...
int uploadUsbExploit(AMRecoveryModeDevice device, const char* fileName)
{
	CFStringRef cfstrFilename = __CFStringMakeConstantString(fileName);
...
int sendCommand(AMRecoveryModeDevice device, const char* cmd)
{
